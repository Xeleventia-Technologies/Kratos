...
{
    Task<Response> GetForUserAsync(long userId, FilterParams filters, CancellationToken cancellationToken);
}

public class Repository([FromServices] DatabaseContext database) : IRepository
{
    public async Task<Response> GetForUserAsync(long userId, FilterParams filters, CancellationToken cancellationToken)
    {
